...
/* Keccak-f[1600] offloaded to the tightly-coupled accelerator through the
 * hand-written assembly routine in keccak_permute.s (xor5 / xandn / rxri custom
 * instructions). It exports the same symbol, so the callers below are unchanged. */
extern void KeccakF1600_StatePermute(uint64_t state[25]);
#elif defined(USE_COPROCESSOR_AXI)
 ...
 * at DEV_KECCAK (0x5000_1000). The 25 state lanes are pushed to the DATA
 * registers, the permutation is kicked off through CSREG.START and polled on
 * CSREG.DONE, then the permuted state is read back. Mirrors the Ascon AXI
 * offload in ascon.h and keeps the same KeccakF1600_StatePermute symbol so the
 * FIPS-202 callers below are unchanged. */
static void KeccakF1600_StatePermute(uint64_t state[25])
{
   ...
   * state. Same race as documented in the Ascon offload path. */
  __asm__ volatile("fence" ::: "memory");

  /* Kick off the permutation. Writing the full word with START=1 also clears
   * DONE (bit 1 = 0), arming the DONE poll below for this invocation. */
