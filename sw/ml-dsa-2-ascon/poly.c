...
*              output stream of Ascon-XOF(seed|nonce)
*
...
*              output stream from Ascon-XOF(seed|nonce)
*
* Arguments:   - poly *a: pointer to output polynomial
...
void poly_challenge(poly *c, const uint8_t seed[CTILDEBYTES]) {
  ...
  uint64_t signs;
  uint8_t buf[ASCON_XOF_RATE];
  ascon_state state;

  ascon_xof_init(&state);
  ascon_xof_absorb(&state, seed, CTILDEBYTES);
  ascon_xof_finalize(&state);
  ascon_xof_squeezeblocks(buf, 1, &state);
